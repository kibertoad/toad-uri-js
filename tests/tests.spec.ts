...
test('Convert IRI to URI', () => {
  //example from RFC 3987
  strictEqual(
    URI.serialize(URI.parse('uri://www.example.org/red%09ros\xE9#red', IRI_OPTION)),
    'uri://www.example.org/red%09ros%C3%A9#red',
  )

  //Internationalized Domain Name conversion via punycode example from RFC 3987
  strictEqual(
    URI.serialize(URI.parse('uri://r\xE9sum\xE9.example.org', { iri: true, domainHost: true }), {
      domainHost: true,
    }),
...
test('Convert URI to IRI', () => {
  ...
  strictEqual(
    URI.serialize(URI.parse('uri://xn--99zt52a.example.org/%e2%80%ae'), IRI_OPTION),
    'uri://xn--99zt52a.example.org/%E2%80%AE',
  ) //or uri://\u7D0D\u8C46.example.org/%E2%80%AE

  //Internationalized Domain Name conversion via punycode example from RFC 3987
  strictEqual(
    URI.serialize(URI.parse('uri://xn--rsum-bpad.example.org', { domainHost: true }), {
      iri: true,
      domainHost: true,
    }),
...
if (URI.SCHEMES.http) {
  ...
  test('HTTP Equals', () => {
    //test from RFC 2616
    strictEqual(
      URI.equal('http://abc.com:80/~smith/home.html', 'http://abc.com/~smith/home.html'),
      true,
    )
    strictEqual(
      URI.equal('http://ABC.com/%7Esmith/home.html', 'http://abc.com/~smith/home.html'),
      true,
    )
    strictEqual(
      URI.equal('http://ABC.com:/%7esmith/home.html', 'http://abc.com/~smith/home.html'),
...
if (URI.SCHEMES.urn) {
  ...
  test('URN NID Override', () => {
    const components = URI.parse('urn:foo:f81d4fae-7dec-11d0-a765-00a0c91e6bf6', { nid: 'uuid' })
    strictEqual(components.error, undefined, 'errors')
    strictEqual(components.scheme, 'urn', 'scheme')
    strictEqual(components.path, undefined, 'path')
    strictEqual(components.nid, 'foo', 'nid')
    ...
    strictEqual(
      URI.serialize(components2, { nid: 'uuid' }),
...
if (URI.SCHEMES.mailto) {
  ...
  test('Mailto Serialize', () => {
    ...
    strictEqual(
      URI.serialize({ scheme: 'mailto', to: ['infobot@example.com'], body: 'current-issue' }),
      'mailto:infobot@example.com?body=current-issue',
    )
    strictEqual(
      URI.serialize({ scheme: 'mailto', to: ['infobot@example.com'], body: 'send current-issue' }),
    ...
    strictEqual(
      URI.serialize({ scheme: 'mailto', to: ['"\\\\\\"it\'s\\ ugly\\\\\\""@example.org'] }),
      "mailto:%22%5C%5C%5C%22it's%5C%20ugly%5C%5C%5C%22%22@example.org",
    )
    strictEqual(
      URI.serialize({ scheme: 'mailto', to: ['user@example.org'], subject: 'caf\xE9' }),
...
if (URI.SCHEMES.ws) {
  ...
  test('WS Serialize', () => {
    ...
    strictEqual(
      URI.serialize({ scheme: 'ws', host: 'example.com', resourceName: '/foo?bar' }),
    ...
    strictEqual(
      URI.serialize({ scheme: 'ws', host: 'example.com', resourceName: '/foo?bar', secure: false }),
      'ws://example.com/foo?bar',
    )
    strictEqual(
      URI.serialize({ scheme: 'ws', host: 'example.com', resourceName: '/foo?bar', secure: true }),
...
if (URI.SCHEMES.wss) {
  ...
  test('WSS Serialize', () => {
    ...
    strictEqual(
      URI.serialize({ scheme: 'wss', host: 'example.com', resourceName: '/foo?bar' }),
      'wss://example.com/foo?bar',
    )
    strictEqual(
    ...
    strictEqual(
      URI.serialize({ scheme: 'wss', host: 'example.com', resourceName: '/foo?bar', secure: true }),
      'wss://example.com/foo?bar',
    )
  })
